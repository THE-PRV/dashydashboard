...
\begin{warnbox}
If IIS was installed \emph{before} the Hosting Bundle, you must restart IIS to register the ASP.NET Core Module. Skip this step if you installed the Hosting Bundle before IIS.
\end{warnbox}

\begin{enumerate}
  \item Click Start, type \textbf{IIS Manager}, and open \emph{Internet Information Services (IIS) Manager}.
  ...
  \item If it is \emph{not} listed:
  \begin{enumerate}[label=\alph*.]
    \item Click Start, type \textbf{Command Prompt}, right-click it and choose \textbf{Run as administrator}.
    \item Type \texttt{iisreset} and press Enter.
    \item Wait for the confirmation message (\emph{Internet services successfully restarted}).
    \item Return to IIS Manager, refresh the Modules list, and confirm \textbf{AspNetCoreModuleV2} now appears.
  \end{enumerate}
\end{enumerate}
